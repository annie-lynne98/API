...
";
$employee_list = explode("," ,strtoupper($wambua_corrupted_employees));

$employee_list2 = array_splice(array_unique($employee_list), 0, 7);



echo "<table border='3'>";
echo "<tr>";
foreach ($employee_list2 as $value ) {
...
$employee_lists = explode(",",strtoupper($wambua_corrupted_employees));
$employee_list3 = array_splice(array_unique($employee_lists), 8, 254);
$rows_array = array_chunk($employee_list3,7);

foreach ($rows_array as $tablecells ) {
  echo '<tr>';
  $x = 1;
while( $x <= 7){
    foreach($tablecells as $tablecell):

      $tablecell2 = preg_replace('/[@#^$&%>>;+=<\/-]+/', '', $tablecell);

    $x++;
echo "<td>{$tablecell2}</td>";
    endforeach;
